...
function plugin(options) {
  ...
  function createFiles(files, file, alias, baseurl) {
    ...
    if (options.redirect) {
      ...
      const redirectBuffer = new Buffer(`
        <meta http-equiv="refresh" content="0; url=${baseurl}/${file.path}">
        <link rel="canonical" href="${baseurl}/${file.path}" />
      `);
    ...
    Object.keys(files)
      ...
      .forEach(file => {
        if (Object.prototype.toString.call(file.aliases) === '[object Array]') {
          ...
          createFiles(files, file, './' + file.aliases, metalsmith.metadata().site.baseurl);
        }
      });
    done();
  };
